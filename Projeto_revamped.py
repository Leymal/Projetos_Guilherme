...
# Função do menu do conjunto verdade, printa as opções e pede um input (opção) do usuário
def conjunto_verdade (conjunto_verdade):
    conjunto_universo = input ("Qual o conjunto universo?\n"
                               "1 - Inteiro\n"
                               "2 - Natural\n"
                               "3 - Real?\n")
    
    # Pede a primeira e a segunda proposição, usando U para União, I para Interseção, X para disjunção exclusiva e ~ para negação
    proposicao1 = input ("Qual a primeira proposição? (não use espaço, para representar União utilize U" \
                         ", para representar Interseção utilize I, para representar a disjunção exclusiva utilize X, para representar a negação utilize ~)\n")
    proposicao2 = input ("Qual a segunda proposição? (não use espaço, para representar União utilize U" \
                         ", para representar Interseção utilize I, para representar a disjunção exclusiva utilize X, para representar a negação utilize ~)\n")
    
    # Verifica se as proposições são válidas (não complexas)
    if len(proposicao1) != len(proposicao2):
        if "==" in proposicao1 or "==" in proposicao2:
            print("Proposições não são válidas.\n")
